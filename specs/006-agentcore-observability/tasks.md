...
<!-- sequential -->
- [x] T001 Add observability resources to /home/dhamijag/playground/strands-demo/infra/agentcore/template.yaml — three `AWS::Logs::LogGroup` resources with RetentionInDays: 30 and DeletionPolicy: Delete: Runtime application logs, Runtime usage logs, Identity application logs. Also add three log group ARN outputs. (Note: Transaction Search `AWS::Logs::ResourcePolicy` and `AWS::XRay::TransactionSearchConfig` were omitted from CFN — they are account-level singletons already active via CLI. Identity ARN is not a CFN output — it is derived from the Runtime via `get-agent-runtime` API.)
